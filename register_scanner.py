...
def write_single_hold_regiser(ip,valid_list,permission_list):
	write_confirm_list=[]
	write_confirm_data=[]
	client=ModbusClient(host=ip,port=502,auto_open=True,auto_close=True,timeout=10)
...
def write_single_coil_regiser(ip,coil_valid_list):
...
def read_valid_registers(ip,reg):
	valid_list=[]
	data_list=[]
	permission_list=[]
	client=ModbusClient(host=ip,port=502,auto_open=True,auto_close=True,timeout=10)
...
coil_valid_list,coil_data_list,coil_permission_list=read_valid_registers(ip,"coil") #Function Code - 0x01
if coil_data_list:
	print("Writing Coil Status - 0x05")
	write_coil_valid_list,write_coil_data_list,coil_permission_list=write_single_coil_regiser(ip,coil_valid_list) #Function Code - 0x05
else:
...
hold_valid_list,hold_data_list,hold_permission_list=read_valid_registers(ip,"hold") #Function Code - 0x03
#print_details(hold_valid_list,hold_data_list,"Read")
if hold_data_list:
	print("Writing Holding Status - 0x06")
	write_hold_valid_list,write_hold_data_list,hold_permission_list=write_single_hold_regiser(ip,hold_valid_list,hold_permission_list) #Function Code - 0x06
else:
	print("No valid Register Found to do Write Operation")
...
discrete_valid_list,discrete_data_list,discrete_permission_list=read_valid_registers(ip,"discrete")	#Function Code - 0x02
...
input_valid_list,input_data_list,input_permission_list=read_valid_registers(ip,"input")
